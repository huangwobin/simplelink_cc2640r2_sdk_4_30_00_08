...
imgTypes = {'app': 1, 'stack': 2, 'np': 3, 'production': 4, 'remoteapp': 5 }
...
#note that only internal flash pages between 1 and 30 (inclusive) can be updated OTA for int flash OAD
INT_FL_OAD_IMG_B_META_BEGIN = 0x10000      #First addr of on chip OAD img B
INT_FL_OAD_IMG_B_END        = 0x1EFFF
INT_FL_OAD_IMG_A_META_BEGIN = 0x1000
INT_FL_OAD_IMG_A_END        = 0xFFFF
...
if __name__ == "__main__":
    ...
    def addr_is_in_oad_imgspace(addr):
        ...
        if vargs.oadtype == 'offchip':
            ...
            #oad imgA on chip production space is between end of pg1-15
            return addr >= INT_FL_OAD_IMG_A_META_BEGIN and addr <= INT_FL_OAD_IMG_A_END
        else:
            #right now on chip OAD considers App space to be between pg1-30, which includes the stack.
            #assume for now that the user doesn't try to udpate the stack
            return addr >= INT_FL_OAD_IMG_A_META_BEGIN and addr <= INT_FL_OAD_IMG_B_END
    ...
    def argument_sanity_check(vargs, mergedHex):
        ...
        elif vargs.imgtype == 'app' or vargs.imgtype == 'stack' or vargs.imgtype == 'remoteapp':
        ...
        if vargs.meta is not None:
            if vargs.oadtype == 'onchip' and vargs.imgtype == 'production':
                ...
                #by default it resides at 0x1000, however, just be sure it doesn't reside in BIM
    ...
    parser.add_argument('-i', '--imgtype', help="Defines the img type. For Onchip: only app and production are valid. For Offchip: app, stack, prodcution, np and remoteapp are valid",
                        choices=['app', 'stack', 'np', 'production', 'remoteapp'], default='app')
...
#        __builtin__.print = myprint
    ...
    if not(vargs.oadtype == 'offchip' and (vargs.imgtype == 'production' or vargs.imgtype == 'np')):
        # Place metadata, onchip production image expects header at 0x1000
        if vargs.oadtype == 'onchip' and vargs.imgtype == 'production':
        ...
        if metaAddr is None:
            ...
            #if onchip production image, we must start at imgA start with is 0x1000
            if vargs.oadtype == 'onchip' and vargs.imgtype == 'production':
            ...
            if not addr_is_in_oad_imgspace(metaAddr):
                print("Fatal Error: -- Metadata (0x%08X) must be at the start, additionally, must be within APP+STACK bounds. Exiting." % metaAddr)
                sys.exit(1)
            else:
                # Trust the user
                print("Placing metadata at 0x%08X" % metaAddr)

            if metaAddr != 0x14F0 or vargs.oadtype == 'onchip': #TIRTOS_IN_ROM
                startAddr = metaAddr
        ...
        if vargs.oadtype == 'onchip' and vargs.imgtype == 'production':
            asBin = mergedHex.tobinstr(INT_FL_OAD_IMG_A_META_BEGIN + 4, INT_FL_OAD_IMG_A_END)
        else:
            if metaAddr == startAddr :
                asBin = mergedHex.tobinstr(startAddr + 4, endAddr-1)
            else:
                print("Skipping first 4 bytes of metaAddr in crc calculation where crc values are to be stored")
                asBin = mergedHex.tobinstr(startAddr, metaAddr -1)
                asBin = asBin + mergedHex.tobinstr(metaAddr + 4, endAddr-1)
    ...
    # Since startAddr is modified for previous calculations to point to 0x1000,
    # we must set start address back to zero to write the interrupt vectors.
    if vargs.oadtype == 'onchip' and vargs.imgtype == 'production':
        startAddr = 0
