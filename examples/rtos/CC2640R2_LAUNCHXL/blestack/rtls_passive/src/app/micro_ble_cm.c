...
#define BLE_CONSECUTIVE_MISSED_CONN_EVT_THRES       5
                                 ...
                                 uint8_t len, uint8_t *pPayload)
{
  ...
  if (cmMaster == TRUE)
  {
    ...
    connInfo->prevChan = connInfo->currentChan;
    cmMaster = FALSE;
 ...
 */
uint8_t ubCM_startExt(uint32_t accessAddress, uint16_t connInterval, uint8_t hopValue, uint8_t nextChan, uint8_t *chanMap, uint32_t crcInit)
{
  ...
  if (ubCMConnInfo.numHandles <= CM_MAX_SESSIONS)
  {
    ...
    chanSkip = (uint16_t)((640/connInterval) + 1);
 ...
 *                 index will be greater than or equal to CM_MAX_SESSIONS.
 */
uint8_t ubCM_updateExt(uint8_t sessionId, uint32_t accessAddress, uint16_t connInterval, uint8_t hopValue, uint8_t nextChan, uint8_t *chanMap, uint32_t crcInit)
{
  uint8_t i;
  ...
  //RTLSCtrl_sendDebugEvent("ubCM_updateExt, nextChan=", nextChan);
  memcpy(ubCMConnInfo.ArrayOfConnInfo[i].chanMap, chanMap, CM_NUM_BYTES_FOR_CHAN_MAP);
